 ...
 */
		...
		var main = function (ruleContext) {
			...
			var inParamObj = jsonUtil.json2obj(inParams).dataSourceMap[0];
			...
			if(saveTarget == "app"){
				save2Native(routeContext,ruleContext,inParamObj);
			}else{
				upload2Server(routeContext,ruleContext,inParamObj);
			}
			ruleContext.markRouteExecuteUnAuto();
		};
		
		var upload2Server = function(routeContext,ruleContext,inParamObj){
			var type = inParamObj.sourceType;
			if(type == "filePath"){
				uploadFromFilePath(routeContext,ruleContext,inParamObj);
			}else{
				uploadFromUserSelect(routeContext,ruleContext,inParamObj);
			}
		} 
		
		var uploadFromFilePath = function(routeContext,ruleContext,inParamObj){
			//1.从实体中获取本地文件路径
			var type = inParamObj.sourceType;
			...
			if(!sDataSource)	{
				HandleException("请检查实体"+sEntityCode+"是否存在");
			}
		...
		var uploadFromUserSelect = function(routeContext,ruleContext,inParamObj){
			var type = inParamObj.sourceType;
		...
		var save2Native = function(routeContext,ruleContext,inParamObj){
			var type = inParamObj.sourceType;
			var dataSource = GetDataSource(inParamObj.resultEntityCode,routeContext);
			if(!dataSource)	{
				HandleException("请检查实体"+entityCode+"是否存在");
